...
<div x-data="(() => { const fileField = @js($fileField); const label = @js($label); return {
    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-80 z-50 flex items-center justify-center" id="camera-modal-{{ $fileField }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl mx-4 sm:mx-6 w-full max-w-lg" @click.stop>
            ...
            <video x-ref="video" class="w-full aspect-video rounded-xl object-cover mb-4 ring-2 ring-indigo-500 shadow-md" autoplay playsinline></video>
            <div class="flex justify-center gap-4">
                ...
                <button type="button" @click="takePhoto()"
                        class="px-6 py-2 rounded-full font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors duration-200 shadow-lg">Capturer</button>
            </div>
        </div>
    </div>
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            <i class="{{ $icon }} text-indigo-500 mr-2"></i>{{ $label }}
        </label>
        <div class="flex flex-col sm:flex-row items-center gap-4">
            @if ($preview)
                <div class="relative {{ $fileField === 'photoProfil' ? '' : 'flex-grow' }}">
                    <img src="{{ $preview }}" class="{{ $fileField === 'photoProfil' ? 'h-24 w-24 object-cover rounded-full' : 'w-full h-auto object-contain rounded-lg' }} shadow-md ring-2 ring-indigo-300 dark:ring-indigo-700 transition-transform duration-300 hover:scale-105" alt="Preview {{ $label }}">
            ...
            <div class="flex-grow flex flex-col sm:flex-row gap-2">
                <label for="{{ $fileField }}" class="flex-grow text-center cursor-pointer px-4 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 text-indigo-600 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200 border-2 border-dashed border-gray-300 dark:border-gray-600 font-medium">
                    <input id="{{ $fileField }}" wire:model="{{ $fileField }}" type="file" class="sr-only" accept="image/*">
                    <span>Choisir un fichier</span>
                </label>
                <button type="button" @click="openCameraModal('{{ $cameraField }}')" class="w-full sm:w-auto px-4 py-3 rounded-xl bg-indigo-50 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-800 transition-colors duration-200 font-medium whitespace-nowrap">
